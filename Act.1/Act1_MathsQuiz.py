...
correct_sound = pygame.mixer.Sound("correct.wav") # Sound effects
wrong_sound = pygame.mixer.Sound("wrong.wav")

root = tk.Tk()
root.title("Arithmetic Quiz Game")
root.geometry("800x500")
root.resizable(False, False) # Removes option to maximize screen

icon_image = ImageTk.PhotoImage(file="icon.jpg")  # Icon for the app (applies to all pages)
root.iconphoto(False, icon_image)

# Background for start, instructions, and select difficulty page (bg image created by me)
menu_bg = Image.open("menu.png").resize((800, 500), Image.LANCZOS) # Info researched and gathered from GeeksforGeeks
menu_bg_photo = ImageTk.PhotoImage(menu_bg)

instruction_bg = Image.open("ins.png").resize((800, 500), Image.LANCZOS)
instruction_bg_photo = ImageTk.PhotoImage(instruction_bg)

difficulty_bg = Image.open("dif.png").resize((800, 500), Image.LANCZOS)
difficulty_bg_photo = ImageTk.PhotoImage(difficulty_bg)

# Background for the main quiz
quiz_bg = Image.open("quiz.png").resize((800, 500), Image.LANCZOS)
quiz_bg_photo = ImageTk.PhotoImage(quiz_bg)

# Background changes depending on the result
high_bg = Image.open("high.png").resize((800, 500), Image.LANCZOS)
hight_bg_photo = ImageTk.PhotoImage(high_bg)

med_bg = Image.open("med.png").resize((800, 500), Image.LANCZOS)
med_bg_photo = ImageTk.PhotoImage(med_bg)

low_bg = Image.open("low.png").resize((800, 500), Image.LANCZOS)
low_bg_photo = ImageTk.PhotoImage(low_bg)
...
def displayDifficulty(): # User shall select the difficulty level
    ...
    easy_btn = tk.Button(root, text="Easy (1-digit)", width=20, height=2, command=lambda: start_quiz("easy"),
            bg="DarkOliveGreen1", font=("Arial", 12, "bold")) # Easy level button
    easy_btn.place(relx=0.5, rely=0.39, anchor="center") 
    
    moderate_btn = tk.Button(root, text="Moderate (2-digit)", width=20, height=2, command=lambda: start_quiz("moderate"),
            bg="light goldenrod", font=("Arial", 12, "bold")) # Moderate level button
    moderate_btn.place(relx=0.5, rely=0.53, anchor="center") 
        
    advanced_btn = tk.Button(root, text="Advanced (4-digit)", width=20, height=2, command=lambda: start_quiz("advanced"),
...
def displayProblem(): # Displays the question
    ...
    global answer_entry # Boxes for users to enter their answer
...
def start_quiz(selected_level):
    ...
    score = 0 # Resets the score when game is played again
